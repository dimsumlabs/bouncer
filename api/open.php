...
$link->exec($query)
    or $link->exec("UPDATE Users SET count = count + 1$rfid2, last_seen = DATETIME('now') WHERE DATE('now') <= MAX(IFNULL(paid,0),IFNULL(paid_verified,0)) AND password = '$password2'")
